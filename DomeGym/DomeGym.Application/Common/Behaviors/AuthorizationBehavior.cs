...
namespace DomeGym.Application.Common.Behaviors;
    ...
    where TResponse : IErrorOr
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (requiredPermissions.Except(currentUser.Permissions).Any()) return (dynamic)Error.Unauthorized(description: "User is forbidden from taking this action");

        var requiredRoles = authorizationAttributes
            .SelectMany(authorizationAttribute => authorizationAttribute.Roles?.Split(',') ?? [])
            .ToList();

        if (requiredRoles.Except(currentUser.Roles).Any()) return (dynamic)Error.Unauthorized(description: "User is forbidden from taking this action");
